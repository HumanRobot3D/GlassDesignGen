...
def addModifiedCurve(col, empty):
    ...
    #Screw Modifier
    screw_modif = curve_obj.modifier_add('Screw', "SCREW")
    screw_modif.angle = random.uniform(Options.screw_modif_angle[0], Options.screw_modif_angle[1])

    #Solidify Modifier
    solidify_modif = curve_obj.modifier_add('Solidify', "SOLIDIFY")
    ...
    if Options.wave_modifier_presence == True:
        wave_modif = curve_obj.modifier_add('Wave', "WAVE")
        wave_modif.height = random.uniform(Options.wave_modif_height[0], Options.wave_modif_height[1])
        anim_length = scene.frame_end / random.choice(Options.wave_modif_loop_length[0], Options.wave_modif_loop_length[1])
    ...
    if random_remesh == True and random.choice(Options.random_remesh_frequency) == 0:       
        remesh_modif = curve_obj.modifier_add('Remesh', "REMESH")
...
def randomMovementZ(obj_list):
    ...
    for count in range(frequency-1):
        keyframes_list.append((scene.frame_end/frequency)*(count+1))
    ...
    for frame in frames_list:
        rand_locz = (random.uniform(-1, 1) * random_locz_intensity) / frequency #I divide by frequency so objects that moves faster moves less.
        for obj in obj_list:
...
def randomRotation(obj_list, leading_obj = None):
    ...
    if random.choice(Options.random_rot_frequency) == 0:  
        ...
        if leading_obj == None:
            ...
            rotation_amount = math.degrees(leading_obj.euler_rotation[2]) * random.randint(Options.random_rot_intensity[0], Options.random_rot_intensity[1])
...
class MY_OT_GenerateGlassDesign(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for count in range(self.render_number):
            ...
            for count1 in range(random.uniform(num_curves[0], num_curves[1])):
            ...
            for count2 in range(5):
                if random.uniform(0, 1) <= Options.sphere_frequency:
